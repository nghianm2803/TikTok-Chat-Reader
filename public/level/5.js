oS.Init({
    ...
    PicArr: function () {
        var a = oPotatoMine.prototype, b = a.PicArr;
    ...
    StartGame: function () {
        SetHidden($CFun("dSunNum"));
        SetVisible($CFun("tdShovel"));
        NewEle("DivTeach", "div", 0, 0, EDAll);
        oP.Monitor({
            ar: [0], f: function (c) {
                var d, a = oGd.$, b = oS.Chose;
                switch (c) {
                    case 0:
                        innerText($CFun("DivTeach"), "你必须清理一下你的草坪，用铲子挖出那些植物！");
                        NewImg("PointerUD", "images/interface/PointerUP.gif", "top:36px;left:250px", EDAll);
                        oSym.addTask(10, arguments.callee, [++c]);
                        break;
                    case 1:
                        b < 0 && (innerText($CFun("DivTeach"), "点击移除一棵植物！"), ++c);
                        oSym.addTask(10, arguments.callee, [c]);
                        break;
                    case 2:
                        !(a["2_6_1"] && a["3_8_1"] && a["4_7_1"]) ? (innerText($CFun("DivTeach"), "一直挖吧，挖到你的草坪上没有植物！"), ++c) : b > -1 && (innerText($CFun("DivTeach"), "点击铲子挖出那些植物！"), c = 1);
                        oSym.addTask(10, arguments.callee, [c]);
                        break;
                    default:
                        !(a["2_6_1"] || a["3_8_1"] || a["4_7_1"]) ? (function () {
                            SetNone($CFun("DivTeach"), $CFun("PointerUD"));
                            SetVisible($CFun("dSunNum"), $CFun("dFlagMeter"));
            ...
            var c = Math.floor(1 + Math.random() * 5), f = Math.floor(1 + Math.random() * 9), g = GetX(f) - 55,
                e = GetY(c) - 60, b = c + "_" + f, a = oP.FlagZombies, d;
            switch (true) {
                case a > 3:
                    SetStyle(d = $CFun("imgCrater"), {left: g + "px", top: e + "px", zIndex: 3 * c});
